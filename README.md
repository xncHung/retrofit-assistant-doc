...
* Summarize all the retrofit interfaces in the project into a tool window, the experience is similar to the Structure
  tool window, showing all the interfaces in a tree form, you can enter path keywords in the window to filter the
  interfaces, double-click or press enter for quick navigation.
...
* liveTemplate, the plugin has a batch of live templates embedded, for example, you can input 'retrofitApi' in the
  ...
  parameters. This is useful for creating and modifying interfaces.
...
* Interfaces can be grouped by whether they appear in the project or not, and interfaces that have appeared can be
  navigated between documents and code. The grouping function allows you to quickly find new interfaces during version
...
This window summarizes all the interfaces in the project, and displays them in a tree with their URLs, and you can
...
Probably the most common problem encountered by developers in type problems is that some interfaces need to be annotated
...
interfaces need to reuse the fields of Param, and in order to inherit Param, an open keyword is added to the Param
